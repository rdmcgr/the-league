...
<?php
  $teamTotals = [];
  foreach ($config['odds'] as $odd) $teamTotals[$odd['team']] = 0;
...
?>
<section class="card block futures-entry-block"><div class="block-header"><h2>Wagered by Team</h2></div><div class="futures-handle-chart"><?php foreach ($teamTotals as $team => $total): ?><div class="futures-handle-row"><div class="futures-handle-label"><?= esc($team) ?></div><div class="futures-handle-track"><div class="futures-handle-bar" style="width: <?= esc(($total / $maxTeamTotal) * 100) ?>%"></div></div><div class="futures-handle-value"><?= esc(number_format($total)) ?></div></div><?php endforeach; ?><?php if (!$teamTotals): ?><p class="small">No teams available yet.</p><?php endif; ?></div></section>
